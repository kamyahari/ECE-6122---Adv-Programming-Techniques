...
for ECE6122 Lab3/Final Project. It does NOT touch the existing functions.
...
#ifdef USE_LAB3_ASSIMP // don't use this #define, it's only for Lab3 - and is by extension used for the Final Project

bool loadAssImpLab3(const char* path, std::vector<chessComponent>& gchessComponents) {
	//std::cout << "\nLoading model from path: " << path << std::endl;
	...
	//std::cout << "Scene loaded successfully. Number of meshes: " << scene->mNumMeshes << std::endl;
	...
	for (unsigned int i = 0; i < rNode->mNumChildren; i++) {
		...
		/*std::cout << "\nProcessing node: " << cNode->mName.C_Str()
			<< " (has " << cNode->mNumMeshes << " meshes)" << std::endl;*/

		if (cNode->mNumMeshes == 0) {
			std::cout << "Skipping node with no meshes\n";
			continue;
		}
		...
		/*std::cout << "Mesh details:"
			...
			<< "\n  Has positions: " << mesh->HasPositions()
			<< "\n  Has normals: " << mesh->HasNormals()
			<< "\n  Has texture coords: " << mesh->HasTextureCoords(0)
			<< std::endl;*/
		...
		for (unsigned int j = 0; j < mesh->mNumFaces; j++) {
			gChessComponent->addFaceIndices(&(mesh->mFaces[j].mIndices[0]));
		}
		...
		if (mesh->mMaterialIndex >= 0) {
			...
			if (material->GetTexture(aiTextureType_DIFFUSE, 0, &texturePath) == AI_SUCCESS) {
				//std::cout << "Found texture path: " << texturePath.C_Str() << std::endl;
	...
	//std::cout << "\nLoading complete. Loaded " << gchessComponents.size() << " components.\n";
	/*std::cout << "Component IDs loaded:\n";
	for (const auto& comp : gchessComponents) {
